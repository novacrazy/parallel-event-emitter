...
fn main() {
    ...
    emitter.add_listener("test", box || {
        println!("Thread: {:<6} No arguments", thread_id::get());

        Ok(())
    }).unwrap();

    emitter.add_listener_value("test", box |arg: Option<i32>| {
        println!("Thread: {:<6} Expected i32: {:?}", thread_id::get(), arg);

        Ok(())
    }).unwrap();

    emitter.add_listener_value("test", box |arg: Option<&str>| {
    ...
    for i in 0..4 {
        emitter.add_listener_sync("some_sync", box move |arg: Option<&MyType>| {
